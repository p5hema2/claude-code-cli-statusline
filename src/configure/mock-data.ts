...
import type { UsageCache, CachedUsageEntry } from '../types/UsageData.js';
...
export function getMockGitInfo(states: WidgetStates): GitInfo | null {
  if (states.gitBranch === 'notRepo') {
    return null;
  }

  return {
    branch: 'main',
    isDirty: states.gitBranch === 'dirty',
    hasStaged: states.gitBranch === 'dirty',
    hasUntracked: false,
    ahead: states.gitBranch === 'dirty' ? 2 : 0,
    behind: 0,
  };
}
