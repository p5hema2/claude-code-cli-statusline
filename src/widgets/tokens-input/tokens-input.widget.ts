...
export const TokensInputSchema: WidgetSchema = {
  id: 'tokensInput',
  meta: {
  ...
  options: {
    content: { color: 'white' },
    custom: [
      { key: 'label', type: 'text', label: 'Label Prefix', default: 'In', maxLength: 20, placeholder: 'e.g., "In"' },
...
export const TokensInputWidget: Widget = {
  ...
  render(ctx: RenderContext, config?: WidgetConfig): string | null {
    const tokens = ctx.status.token_metrics?.input_tokens;
